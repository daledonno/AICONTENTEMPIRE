...
export default function RootLayout({ children }) {
  ...
  return (
    <html lang="en" className={inter.className}>
      ...
      <body className={`${darkMode ? 'bg-gray-900 text-gray-100' : 'bg-gray-100 text-gray-900'}`}>
        <div className="flex min-h-screen">
          {/* Sidebar */}
          <div 
            className={`${
              collapsed ? 'w-16' : 'w-64'
            } ${darkMode ? 'bg-gray-800' : 'bg-white border-r border-gray-200'} min-h-screen fixed left-0 transition-all duration-300 ease-in-out z-10`}
          >
            <div className="flex items-center justify-between p-4 border-b border-gray-700">
              {!collapsed && (
                <span className="text-xl font-bold text-purple-500">AI Authority</span>
              )}
              <button 
            ...
            <div className={`mt-4 relative ${collapsed ? 'px-2' : 'px-4'}`}>
              <div 
                className={`flex items-center ${collapsed ? 'justify-center' : 'justify-between'} p-2 rounded-lg hover:bg-gray-700 cursor-pointer`}
                ...
                <div className="flex items-center">
                  ...
                  {!collapsed && (
                    <div className="ml-3">
                      ...
                      <div className="text-xs text-gray-400">Switch Profile</div>
                    </div>
                  )}
                </div>
              ...
              {profileMenuOpen && !collapsed && (
                <div className="absolute left-0 right-0 mt-1 mx-4 bg-gray-800 rounded-lg shadow-lg border border-gray-700 z-10">
                  {profiles.map(profile => (
                    <div 
                      key={profile.id}
                      className={`flex items-center p-3 hover:bg-gray-700 cursor-pointer ${profile.id === activeProfile.id ? 'bg-gray-700' : ''}`}
                      onClick={() => {
              ...
              {navItems.map((item) => (
                <Link 
                  ...
                  className={`
                    ...
                    ${pathname === item.path ? 'bg-purple-900 bg-opacity-50 border-l-4 border-purple-500' : ''}
            ...
            <div className="absolute bottom-4 left-0 right-0 flex justify-center">
              <button 
                onClick={toggleTheme} 
                className={`p-2 rounded-full ${collapsed ? '' : 'mx-4'} ${darkMode ? 'bg-gray-700 hover:bg-gray-600' : 'bg-gray-200 hover:bg-gray-300'}`}
              >
